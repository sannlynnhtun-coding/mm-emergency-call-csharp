...
string? status = null, string? townshipCode = null)
	{
		try
		{
			...
			{
				return Result<EmergencyRequestListPaginationResponseModel>.ValidationError("Invalid PageNo.");
			}
		...
		catch (Exception ex)
		{
			...
			return Result<EmergencyRequestListPaginationResponseModel>.SystemError("Internal server error");
		}
	}
}
